...
class RewardTrainer(abc.ABC):
    ...
    def train(self, train_dataset: PreferenceDataset, val_dataset: PreferenceDataset, epoch_multiplier: float = 1.0):
        ...
        with networks.training(self.model):
            self._train(train_dataset, val_dataset, epoch_multiplier)

    @abc.abstractmethod
    def _train(self, train_dataset: PreferenceDataset, val_dataset: PreferenceDataset, epoch_multiplier: float):
...
class CrossEntropyRewardTrainer(RewardTrainer):
    ...
    def _train(self, train_dataset: PreferenceDataset, val_dataset: PreferenceDataset, epoch_multiplier: float = 1.0):
        ...
        if frac > 1.3:
            self._update_weight_decay(2.0)
        else:
            self._update_weight_decay(0.5)
...
class PreferenceComparisons(base.BaseImitationAlgorithm):
    ...
    def train(
        ...
        for i in range(iterations):
            ...
            training_size = int(len(fragments) * (1.0 - self.val_size))
            self.train_dataset.push(fragments[:training_size], preferences[:training_size])
            self.val_dataset.push(fragments[training_size:], preferences[training_size:])
            self.logger.log(f"Dataset now contains {len(self.train_dataset)} samples")
