...
class BasicRewardNet(RewardNet):
    ...
    def forward(self, state, action, next_state, done):
        inputs = []
        if self.use_state:
            inputs.append(th.flatten(th.tensor(state), 1))
        if self.use_action:
            inputs.append(th.flatten(th.tensor(action), 1))
        if self.use_next_state:
            inputs.append(th.flatten(th.tensor(next_state), 1))
        if self.use_done:
            inputs.append(th.reshape(th.tensor(done), [-1, 1]))
